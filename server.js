...
function createServer(config = {}, detectors = {}) {
  ...
  function requireSessionToken(req, res, next) {
    // Pass through if no token is configured (allows local dev without Electron).
...
if (require.main === module) {
  // WFB_SESSION_TOKEN is set automatically by Electron main when spawning this process.
  // In standalone / browser-testing mode it is optional — if not set, all API routes
  // are accessible without a token (open mode). To enable token auth in standalone mode:
  //   PowerShell: $env:WFB_SESSION_TOKEN = node -e "process.stdout.write(require('crypto').randomBytes(32).toString('hex'))"
  //   bash:       export WFB_SESSION_TOKEN=$(node -e "process.stdout.write(require('crypto').randomBytes(32).toString('hex'))")
  if (!process.env.WFB_SESSION_TOKEN) {
    log.warn({
      event:   'startup.no_token',
      message: 'WFB_SESSION_TOKEN is not set — running in OPEN mode (no auth on API routes). Set the env var to enable token auth.',
    });
  }
