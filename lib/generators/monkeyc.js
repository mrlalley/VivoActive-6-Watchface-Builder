...
// Catches bad element data before code generation so users get an immediate,
// readable error instead of a cryptic 5-10s compiler failure.
...
// from lib/allowed-values.js. Changes to valid fonts or shape types are
// synchronized automatically — no manual sync contracts.
const {
  VALID_FONTS: VALID_FONTS_ARRAY,
  VALID_SHAPE_TYPES: VALID_SHAPE_TYPES_ARRAY,
} = require('../allowed-values');

// Wrap arrays in Sets for O(1) membership checks in validation.
const VALID_FONTS = new Set(VALID_FONTS_ARRAY);
const VALID_SHAPE_TYPES = new Set(VALID_SHAPE_TYPES_ARRAY);

const IDENTIFIER_RE = /^[a-zA-Z_][a-zA-Z0-9_]*$/;
const HEX_COLOR_RE  = /^#[0-9a-fA-F]{6}$/;

/**
 * Validate one element's properties before code generation.
 * Focused on code-generation risks (injection, NaN in output) rather than
 * duplicating the structural checks already done by lib/validation.js.
 *
 * @param {Object} el    - Canvas element
 * @param {number} index - Position in elements array (for error messages)
 * @throws {Error} on any property that would produce invalid Monkey C
 */
function validateElement(el, index) {
  const tag = `element[${index}]`;

  // fieldId: must be a valid Monkey C identifier (used in data-fetch map lookups)
  if (!el.fieldId || !IDENTIFIER_RE.test(el.fieldId)) {
    throw new Error(`${tag}.fieldId: invalid identifier "${el.fieldId}"`);
  }

  // shapeType: must be null/undefined (text) or a shape type the generator handles
  if (el.shapeType != null && !VALID_SHAPE_TYPES.has(el.shapeType)) {
    throw new Error(`${tag}.shapeType: unknown shape type "${el.shapeType}"`);
  }

  // font: must be a known Garmin font name; interpolated as Gfx.${el.font}
  // Only applies to text elements — shapes don't use the font property
  if (!el.shapeType && el.font && !VALID_FONTS.has(el.font)) {
    throw new Error(`${tag}.font: unknown font "${el.font}" — expected one of ${[...VALID_FONTS].join(', ')}`);
  }

  // color: must be a valid 6-digit hex color when present
  if (el.color && !HEX_COLOR_RE.test(el.color)) {
    throw new Error(`${tag}.color: invalid hex color "${el.color}" — expected #RRGGBB`);
  }

  // Coordinates and dimensions must be finite numbers (Math.round(NaN) → NaN in output)
  for (const prop of ['x', 'y', 'width', 'height']) {
    const val = el[prop];
    if (val !== undefined && !Number.isFinite(val)) {
      throw new Error(`${tag}.${prop}: expected finite number, got ${val}`);
    }
  }
}

/**
 * Validate all elements before generating any code.
 * @param {Array} elements
 * @throws {Error} on the first invalid element found
 */
function validateGeneratorInputs(elements) {
  if (!Array.isArray(elements)) {
    throw new Error('elements must be an array');
  }
  elements.forEach((el, index) => validateElement(el, index));
}

// Escape a JS string for safe embedding inside a Monkey C double-quoted string literal.
function escapeMcString(s) {
  return String(s)
    .replace(/\\/g, '\\\\')    // backslashes first
    .replace(/"/g, '\\"')      // double quotes
    .replace(/[\r\n\t]/g, ' '); // collapse control chars to space
}
...
function generateDrawCall(el) {
  ...
  const textMap = {
    ...
    timer: `timerVal`,
    utcTime: `utcTime`,
    customLabel: `"${escapeMcString(el.format || el.label || 'Label')}"`,
  };
  ...
  // Apply the same escaping used by customLabel — the catch-all fallback was
  // previously missing it, so a label like 5" band would break the Monkey C string literal.
  const textExpr = textMap[el.fieldId]
    || `"${escapeMcString(el.label || '')}"`;
  return `dc.setColor(${color}, Gfx.COLOR_TRANSPARENT);\n        dc.drawText(${Math.round(el.x)}, ${Math.round(el.y)}, ${font}, ${textExpr}, ${align});`;
}

function generateMonkeyC(elements, background = null) {
  validateGeneratorInputs(elements);
  ...
  // Build onPartialUpdate from actual digital-time elements in the design.
  // Called every second on AMOLED displays for power-saving per-second updates.
  // If the design has no digital time fields, the method is omitted entirely so
  // analog-only faces don't get an unwanted clock stamped over them.
  const DIGITAL_TIME_FIELDS = new Set(['hours', 'minutes', 'seconds', 'amPm']);
  const digitalTimeEls = sorted.filter(el => DIGITAL_TIME_FIELDS.has(el.fieldId));

  const partialTimeExpr = {
    hours:   `clockTime.hour.format("%02d")`,
    minutes: `clockTime.min.format("%02d")`,
    seconds: `clockTime.sec.format("%02d")`,
    amPm:    `(clockTime.hour < 12) ? "AM" : "PM"`,
  ...
  const partialUpdateMethod = digitalTimeEls.length === 0 ? '' : (() => {
    const clears = digitalTimeEls.map(el => {
      const color = colorLiteral(el.color);
      const font  = `Gfx.${el.font || 'FONT_MEDIUM'}`;
      const align = el.align === 'center' ? 'Gfx.TEXT_JUSTIFY_CENTER | Gfx.TEXT_JUSTIFY_VCENTER'
        : el.align === 'right' ? 'Gfx.TEXT_JUSTIFY_RIGHT | Gfx.TEXT_JUSTIFY_VCENTER'
      ...
      // Clear a rectangle matching the element's bounding box (center-origin) with 4px padding
      const hw = Math.round((el.width  || 60) / 2) + 4;
      const hh = Math.round((el.height || 40) / 2) + 4;
      return `dc.setColor(Gfx.COLOR_BLACK, Gfx.COLOR_BLACK);
            dc.fillRectangle(${x - hw}, ${y - hh}, ${hw * 2}, ${hh * 2});
            dc.setColor(${color}, Gfx.COLOR_TRANSPARENT);
            dc.drawText(${x}, ${y}, ${font}, ${partialTimeExpr[el.fieldId]}, ${align});`;
    }).join('\n            ');
...
${sunMethodBody}
    ...
    function onUpdate(dc) {
        ...
        dc.clear();
${background !== null
  ? (background.aod && background.aod.variant === 'dimmed'
    ? `        if (System.getDeviceSettings().isAwake) {
            dc.drawBitmap(0, 0, Rez.Drawables.WatchBackground);
        } else {
            dc.drawBitmap(0, 0, Rez.Drawables.WatchBackgroundAOD);
        }\n`
    : '        dc.drawBitmap(0, 0, Rez.Drawables.WatchBackground);\n')
  : ''}        try {
            var clockTime = Sys.getClockTime();
