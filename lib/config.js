...
const { detectSdkSync, SdkNotFoundError } = require('./sdk-detect');
...
function scanForLatestSdk(basePath) {
  try {
    ...
    // Sort using numeric comparison so '9.10.0' > '9.2.0' (localeCompare without
    // numeric:true would sort '9.10.0' before '9.2.0' because '1' < '2' lexicographically).
    const sdkDirs = dirs
...
/**
 * @private
 * Get the default SDK bin directory using the canonical detectSdkSync() resolver.
 * Falls back to a platform-specific default path if no SDK is found, so callers
 * receive a non-null string and can surface a meaningful "SDK not found" error at
 * build time rather than at config resolution time.
 *
...
function getDefaultSdkBasePath() {
  try {
    ...
    if (!(err instanceof SdkNotFoundError)) throw err;
    // No SDK found — return a platform-aware default path so callers can report a
    // clear "monkeyc not found" error at build time with the expected path.
    const home = os.homedir();
    if (process.platform === 'win32') {
      return path.join(process.env.APPDATA || path.join(home, 'AppData', 'Roaming'),
        'Garmin', 'ConnectIQ', 'Sdks');
    } else if (process.platform === 'darwin') {
      return path.join(home, 'Library', 'Application Support', 'Garmin', 'ConnectIQ', 'Sdks');
    }
    return path.join(home, '.local', 'share', 'Garmin', 'ConnectIQ', 'Sdks');
  }
}
...
/**
 * @private
 * Get the platform-aware default designs directory.
 * Used as fallback when not provided by electron/main.js (dev mode, web mode).
 *
 * @returns {string} ~/.watchfacebuilder/designs on all platforms
 */
function getDefaultDesignsDir() {
  return path.join(os.homedir(), '.watchfacebuilder', 'designs');
}

/**
 * @private
 * Get the platform-aware default backgrounds directory for user-imported images.
 * In Electron mode electron/server-manager.js passes the real userData path.
 * In standalone mode this default is used.
 *
 * @returns {string} ~/.watchfacebuilder/wfb-backgrounds on all platforms
 */
function getDefaultBackgroundsDir() {
  return path.join(os.homedir(), '.watchfacebuilder', 'wfb-backgrounds');
}
 ...
 *
 * @param {Object} overrides - { sdkBin, devKey, exportDir, tempDir, designsDir } from caller
...
function getConfig(overrides = {}, detectors = {}) {
  ...
  const DESIGNS_DIR    = overrides.designsDir    || process.env.GARMIN_DESIGNS_DIR    || getDefaultDesignsDir();
  const BACKGROUNDS_DIR = overrides.backgroundsDir || process.env.GARMIN_BACKGROUNDS_DIR || getDefaultBackgroundsDir();
  ...
  try {
    ...
    log.warn({ dir: EXPORT_DIR, reason: err.message }, 'Cannot create export directory');
  }
  ...
  try {
    ...
    log.warn({ dir: TEMP_DIR, reason: err.message }, 'Cannot create temp directory');
  }
...
// Public API surface
module.exports = { getConfig, scanForLatestSdk };
