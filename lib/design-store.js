...
const { DesignNotFoundError, DesignCorruptedError } = require('./errors');
 ...
 * Reads files concurrently via Promise.all to avoid blocking the event loop.
 *
 * @param {string} designsDir - Directory containing designs
 * @returns {Promise<Array<{ name: string, file: string, savedAt: string, elementCount: number }>>}
 */
async function listDesigns(designsDir) {
  try {
    if (!fs.existsSync(designsDir)) {
      return [];
    }

    const files = (await fs.promises.readdir(designsDir)).filter(f => f.endsWith('.json'));
    const results = await Promise.all(
      files.map(async (file) => {
        try {
          const data = JSON.parse(
            await fs.promises.readFile(path.join(designsDir, file), 'utf8')
          );
          return {
    ...
    const designs = results.filter(Boolean);
...
function loadDesign(designsDir, filename) {
  try {
    ...
    // path.sep suffix is required — without it, startsWith() accepts sibling
    // directories that share the same name prefix (e.g. .../designs-evil/).
    // Pattern matches electron/main.js lines 372-373 and 415-416.
    const resolved           = path.resolve(filePath);
    const designsDirResolved = path.resolve(designsDir);
    if (!resolved.startsWith(designsDirResolved + path.sep)) {
      throw new Error('Access denied: path outside designs directory');
    }

    if (!fs.existsSync(filePath)) {
      logError('design-store:not-found', { file: sanitized });
      throw new DesignNotFoundError(filePath);
    }
    ...
    try {
      ...
      throw new DesignCorruptedError(filePath, parseErr.message);
    }
    ...
    // Always load. The canvas will show red outlines and disable export/preview
    // reactively if any element is out of bounds — no blocking confirmation needed.
    const result = {
    ...
    if (validationWarning) {
      result.validationWarning = validationWarning; // informational only — highlights element
    }

    return result;
  } catch (err) {
    logError('design-store:load-failed', { reason: err.message });
    // Re-throw typed errors as-is so route handlers can classify them with instanceof
    if (err instanceof DesignNotFoundError || err instanceof DesignCorruptedError) {
      throw err;
    }
