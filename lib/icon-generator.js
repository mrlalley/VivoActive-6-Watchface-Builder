...
// Launcher icon size is defined separately since icon-generator is in lib/
const LAUNCHER_ICON_SIZE = 54;

// Precompute CRC32 lookup table once at module load time (constant)
const CRC32_TABLE = (() => {
  const table = [];
  for (let i = 0; i < 256; i++) {
    let c = i;
...
// Generate a PNG icon with solid blue background (matching the current icon style)
function generate54x54Icon() {
  const width = LAUNCHER_ICON_SIZE;
  const height = LAUNCHER_ICON_SIZE;
  const blue_r = 0;
  const blue_g = 0x88;
  const blue_b = 0xcc;
