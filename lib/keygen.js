...
function generateKey(outputPath) {
  return new Promise((resolve, reject) => {
    ...
    // Generate RSA-4096 key with 60-second timeout via Promise.race().
    // timeoutHandle is declared here so .then()/.catch() can clearTimeout it
    // regardless of which branch of the race wins.
    let timeoutHandle;
    Promise.race([
      new Promise((resolveKeyGen, rejectKeyGen) => {
        crypto.generateKeyPair(
          ...
          (err, publicKey, privateKey) => {
            if (err) {
              rejectKeyGen(new Error(`Key generation failed: ${err.message}`));
              return;
            }
            ...
            try {
              ...
              if (writeErr.code === 'EACCES') {
                rejectKeyGen(new Error(`Cannot write to ${absPath} — check permissions`));
              } else {
                rejectKeyGen(new Error(`Write failed: ${writeErr.message}`));
              }
              return;
            }
            ...
            if (!validation.valid) {
              rejectKeyGen(new Error(`Key written but validation failed: ${validation.reason}`));
              return;
            }

            resolveKeyGen({ success: true, path: absPath });
          }
        );
      }),
      new Promise((_, rejectTimeout) => {
        timeoutHandle = setTimeout(
          () => rejectTimeout(new Error('Key generation timeout: operation exceeded 60 seconds. Try again or check system resources.')),
          60000
        );
      }),
    ]).then((result) => {
      clearTimeout(timeoutHandle); // cancel the 60s timer so the event loop can exit
      resolve(result);
    }).catch((err) => {
      clearTimeout(timeoutHandle); // cancel timer on keygen error or timeout
      // Clean up partial file on any error (including timeout)
      try {
        if (fs.existsSync(absPath)) {
          fs.unlinkSync(absPath);
        }
      } catch (cleanupErr) {
        // Ignore cleanup errors; user error message is sufficient
      }
      reject(err);
    });
  });
